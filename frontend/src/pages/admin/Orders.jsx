...
export default function AdminOrders() {
  ...
  return (
    <div>
      ...
      {/* Nivel 2: Filtros */}
      <div className="ituka-card p-6 mb-8 flex flex-col md:flex-row gap-4 items-center dark:bg-[#0F172A] dark:border-white/10">
        <SearchInput 
          value={searchTerm} 
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-ituka-border/60 dark:divide-white/10">
              ...
              {filteredOrders.map(order => (
                <tr key={order._id} className="hover:bg-ituka-cream-soft dark:hover:bg-white/5 transition-colors group">
                  <td className="p-6 pl-8">
                    <div className="flex flex-col">
                      <span className="font-mono text-xs font-bold text-stone-800 dark:text-stone-100 tracking-wide">
                        #{order._id.slice(-6).toUpperCase()}
                      </span>
                      <span className="text-[10px] text-stone-400 mt-1 font-medium">
                        {formatDate(order.createdAt, prefs)}
                      </span>
                    </div>
                  </td>
                  <td className="p-6">
                    <div className="font-bold text-ituka-text dark:text-stone-100 text-sm">{order.user?.name || 'Desconocido'}</div>
                    <div className="text-[10px] text-stone-400">{order.user?.email}</div>
                  </td>
                  <td className="p-6">
                    <div className="flex items-center gap-2 text-sm text-stone-600 dark:text-stone-300">
                      <CreditCard className="w-4 h-4 text-stone-300 dark:text-stone-400" />
                      <span className="capitalize font-medium">{order.paymentMethod || 'Pendiente'}</span>
                    </div>
                  </td>
                  <td className="p-6">
                    <div className="flex items-center gap-2 text-sm text-stone-600 dark:text-stone-300">
                      {order.deliveryMethod === 'recogida' ? <MapPin className="w-4 h-4 text-stone-300 dark:text-stone-400" /> : <Truck className="w-4 h-4 text-stone-300 dark:text-stone-400" />}
                      <span className="capitalize font-medium">{order.deliveryMethod || 'Pendiente'}</span>
                    </div>
                  </td>
                  <td className="p-6 text-ituka-text dark:text-stone-100 font-bold text-sm font-serif">${order.total}</td>
                  <td className="p-6">
                    ...
                    {canUpdateOrders && (
                      <div className="relative group">
                        <select 
                          ...
                          className="appearance-none pl-3 pr-8 py-2 border border-stone-200 dark:border-white/10 rounded-xl text-xs bg-white dark:bg-white/5 hover:border-ituka-green focus:outline-none focus:ring-2 focus:ring-ituka-green/20 cursor-pointer transition-colors font-bold text-stone-600 dark:text-stone-200 shadow-sm"
                        >
                          {STATUSES.map(s => (
